...
# --- Local Imports (V2 Logic applied here) ---
from graph import get_hero_graphs
from gene import get_gene_intelligence

# ============================================================
# 1. CORE LOGIC & TRAINING (RISK PROFILE ENGINE)
# ============================================================
...
def get_artifacts():
    print("System: Loading artifacts and training models...")
    ...
    # --- CRITICAL FIX: Calculate 'group_cox' for Graph.py ---
    # This was missing in your previous version, causing the KeyError
    
    # 1. Prepare TCGA Test Groups
    df_tcga_test_km = df_test_s.copy()
    df_tcga_test_km["risk_cox"] = risk_test_cox
    cut_tcga = np.median(risk_test_cox)
    # Assign 'High' or 'Low' based on median split
    df_tcga_test_km["group_cox"] = np.where(df_tcga_test_km["risk_cox"] > cut_tcga, "High", "Low")

    # 2. Prepare METABRIC Groups
    ...
    return {
        ...
        # Helper for Graphs (NOW CONTAINS 'group_cox')
